...
TEST(SignatureCreatorTest, fileWithOneLine_getSignature_hashOfOneLine) {
    ...
    EXPECT_CALL(file, getLine(_))
        .WillOnce(DoAll(SetArgReferee<0>(first_line), Return(true)));
...
TEST(SignatureCreatorTest, fileWithThreeLines_getSignature_hashOfThreeLines) {
    ...
    InSequence s;
    EXPECT_CALL(file, getLine(_))
        .WillOnce(DoAll(SetArgReferee<0>(lines[0]), Return(true)));
    EXPECT_CALL(file, getLine(_))
        .WillOnce(DoAll(SetArgReferee<0>(lines[1]), Return(true)));
    EXPECT_CALL(file, getLine(_))
        .WillOnce(DoAll(SetArgReferee<0>(lines[2]), Return(true)));
    EXPECT_CALL(file, getLine(_)).WillOnce(Return(false));

    std::size_t expected_hash = 0;
    std::for_each(lines.begin(), lines.end(), [&expected_hash](const std::string& line) {
        expected_hash += std::hash<std::string>{}(line);
    });
