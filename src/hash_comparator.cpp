...
void searchFirstEqualHash(size_t *base_index, size_t *position, HashCreator &new_file, HashCreator &base, size_t *base_shift, size_t *base_hash)
{
...
bool isAddAtTheEnd(HashCreator &hash_creator)
{
    return hash_creator.isNextHash();
}

bool isRemoveLastPart(HashCreator& hash_creator) {
    return !hash_creator.isNextHash();
}

bool isNotFoundHash(const size_t position) {
    return position == -1;
}
...
bool isBaseBeforeNewFile(const size_t base_index, const size_t position) {
    return base_index > position;
}

bool isAddAtBeginning(const size_t position, const size_t base_index) {
    return position == 0 || base_index == 0;
}

bool isChangeInTheMiddle(const size_t base_index, const size_t position, const size_t base_shift, const size_t window_size) {
    return base_index != position && base_shift != window_size - 1;
}

bool isAtBeginnig(const size_t position) {
    return position == 0;
}
...
HashComparator::HashComparator(HashCreator &base, HashCreator &new_file) {
    ...
    while (base.isNextHash()) {
        ...
        if (base_hash != new_file_hash) {
            ...
            if (isNotFoundHash(position)) {
                ...
                addOperation({chng, base_begin + window_size - 1, base.getSizeContent() -1 , new_file_index + window_size - 1, new_file.getSizeContent() - 1});
                return;
            }
    ...
    if (isAddAtTheEnd(new_file)) {
        debug("It is added at the end");
        const size_t position = base_index + window_size;
        addOperation({add, position, position, position, new_file.getSizeContent() - 1});
    }
...
void HashComparator::addOperation(const Operation &operation)
{
    diffrences_.push_back(operation);
}
